...
import { route, includeHTML } from "./apex/index.js";
...
$(document).ready(function () {
  ...
  function init_template() {
    ...
    if ($(".check-visited").length) {
      setTimeout(function () {
        var getScroll = localStorage.getItem(
          "z_Appkit-Visited-Scroll-Position"
        );
    ...
    if (visitDetection.length) {
      ...
      if (!lastVisitValue) {
        ...
        localStorage.setItem(
          "Appkit-Last-Visited",
          "<br>Your last visit was " + strDate + " at " + strTime
        );
      } else {
    ...
    function highlightLoader() {
      $("[data-highlight-style]").on("click", function () {
        ...
        $("#highlight").remove();
        $("head").append(
          $('<link rel="stylesheet" id="highlight" type="text/css" />').attr(
            "href",
            getHighlight
          )
        );
      ...
      $('a[data-highlight-style="' + rememberHighlight + '"]').addClass(
        "menu-active"
      );
      $("head").append(
        $('<link rel="stylesheet" id="highlight" type="text/css" />').attr(
      ...
      if (isMobile.Android()) {
        function showInstallPrompt() {
          if ($("#menu-install-pwa-android, .add-to-home").length) {
            if (
              localStorage.getItem("Appkit-PWA-Prompt") != "install-rejected"
            ) {
              setTimeout(function () {
                $(".add-to-home").addClass(
                  "add-to-home-visible add-to-home-android"
                );
                $("#menu-install-pwa-android, .menu-hider").addClass(
                  "menu-active"
                );
              }, 4500);
        ...
        window.addEventListener("appinstalled", (evt) => {
          $("#menu-install-pwa-android, .menu-hider").removeClass(
            "menu-active"
          );
        });
      }
  ...
  if (isDevMode === true) {
    if (!$(".reloader").length) {
      $("body").append(
        '<a href="#" class="reloader" style="opacity:1; position:fixed; background-color:#000; color:#FFF; z-index:9999; bottom:100px; left:50%; margin-left:-70px; border-radius:10px; width:140px; line-height:40px; text-align:center;">Developer Mode</a>'
      );
    }
